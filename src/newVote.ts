export function newVote(): void{
    const tBody = document.getElementById('answers__tbody')
    const addAnswerBtn = document.getElementById('add-answer')
    let answerRows: HTMLCollection
    let answerRowEl: Node
...
// Add listener and handler for addAnswerBtn, calls in next eventLoop iteration
    ...
    function findInput(nodes: NodeList): HTMLInputElement {
        let input: HTMLInputElement | null
        Array.from(nodes).forEach((node) => {
            if (node.nodeName === 'INPUT') {
                input = <HTMLInputElement>node
